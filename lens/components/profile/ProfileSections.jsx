...
import { Map, Trophy, Activity, Bookmark } from "lucide-react";
 ...
 * message when no data exists for that section (Req 12.4). Uses the Tabs
                ...
                {hasAchievements ? (
                    <ul className="space-y-2 py-4">
                        {achievements.map((item, idx) => (
                            <li key={item.id ?? idx} className="flex items-center gap-2 text-sm text-text-primary">
                ...
                {hasBookmarks ? (
                    <ul className="space-y-2 py-4">
                        {bookmarks.map((item, idx) => (
                            <li key={item.id ?? idx} className="text-sm text-text-primary">
                                <Bookmark className="mr-1.5 inline h-4 w-4 text-text-secondary" aria-hidden="true" />
