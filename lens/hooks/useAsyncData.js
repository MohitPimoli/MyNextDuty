 ...
 *
 * Timing enforced (Req 13.1, 13.3, 13.4, 15.1):
 ...
 *
 * Result mapping:
 *   - resolve with data → "loaded"; resolve with zero items → "empty" (Req 14.1)
 *   - reject or timeout → "error" with no partial/stale data (Req 15.1)
 *
 * Retry (Req 15.2, 15.3): the returned `retry` re-invokes the fetcher with the
...
export const useAsyncData = (fetcher, params, options = {}) => {
    ...
    // Params captured at fetch start, reused verbatim by retry (Req 15.2).
    ...
    const run = useCallback(
        (requestParams) => {
            ...
            // show delay so fast fetches never flash a skeleton (Req 13.1).
            setState({
                status: STATUS.LOADING,
                data: null,
                error: null,
            ...
            // once the skeleton has actually been shown (Req 13.3).
            const settle = (next) => {
                if (!isCurrent()) {
                    return;
                }
            ...
            // Per-context deadline: transition to error on timeout (Req 13.4, 15.1).
    ...
    // Retry reuses the exact params captured at the last fetch start (Req 15.2).
