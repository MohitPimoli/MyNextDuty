...
/**
 * Format a rating to one decimal place, clamped to the 0.0–5.0 scale.
 *
...
/**
 * Clamp a value to an integer within the inclusive range [min, max].
 *
...
export const clampInt = (n, min, max) => {
  ...
  const value = typeof n === "number" && Number.isFinite(n) ? Math.trunc(n) : lower;
  return Math.min(upper, Math.max(lower, value));
};

/**
 * Resolve an image URL, falling back to a placeholder when absent.
 *
...
/**
 * Return at most `max` items from a list.
 *
